...
Registry of this project's Use Cases. One row per UC. The detail for each UC
lives in `../modules/<Module>/UC-*.md`. IDs follow `UC-<DomainCode>-<Name>`.

| UC ID             | Title                  | Domain   | Primary Actor | Status |
|-------------------|------------------------|----------|---------------|--------|
| UC-ID-Login       | Magic-link login       | Identity | End User      | Draft  |
| UC-BI-ViewInvoice | Customer views invoice | Billing  | Customer      | Draft  |
